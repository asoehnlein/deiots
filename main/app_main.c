...
void app_main(void)
{
    ...
    if (provisioned == false) {
        ...
        xReturned = xTaskCreatePinnedToCore(&start_firmware_check, "start_firmware_check", MYAPP_RTOS_TASK_STACK_SIZE_LARGE, NULL, RTOS_TASK_PRIORITY_NORMAL, NULL, 1);
        if (xReturned == pdPASS) {
            printf("OK Task firmware_check has been created, and is running right now\n");
        }
        start_mqtt();
